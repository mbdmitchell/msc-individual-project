...
dredd_variant_name="dawn-mutant"
...
source ${mutation_testing_directory}/setup_dredd_environment.sh  # Load the Dredd environment
...
build_dawn() { # Function to set up the build directory and generate compile commands
  ...
  cmake ../.. -G Ninja \
      ...
      -DCMAKE_EXE_LINKER_FLAGS="-L${GOOGLE_BENCHMARK_DIR}/lib"
...
${DREDD_EXECUTABLE} \
  ...
  --mutation-info-file "${mutation_testing_directory}/mutant-info.json" \
  "${FILES_TO_MUTATE[@]}" > "${mutation_testing_directory}/dredd_output.log" 2>&1

# Check if the JSON file was generated
if [ -f "${mutation_testing_directory}/mutant-info.json" ]; then
...
build_dawn  # rebuild (now with the mutations)
