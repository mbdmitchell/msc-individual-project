...
class CodeBuilder(ABC):
        ...
        def case_str(ix_: int, code_str: str = None) -> str:
            ...
            if isinstance(self.langauge, WASMLang):
                ...
                return self.language.switch_case_str_pre_format().format(
                    ...
                    possible_switch_break=self.switch_break_str(cases[ix_], is_fallthrough,switch_label_num))
        ...
        if not isinstance(self.langauge, WASMLang):
            ...
            code = """
                                (block (local.get $control_val)
                                    ...
                                    (br {label})
                                )""".format(br_table=WASMLang.build_br_table(cases),
                                            label=WASMLang.switch_label(switch_label_num))
    ...
    def build_code(self) -> str:
        ...
        program: str = self.full_program_aux(raw_code)
